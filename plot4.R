...
png("plot4.png", width=480, height=480) #set image size to 480 x 480
attach(ddat)
par(mfrow=c(2,2))
#First plot
plot(ddat$Global_active_power, type="l", ylab="Global Active Power", xlab="", xaxt = "n")
axis(1, xaxis, labels=xaxislabels)
#Second plot
plot(ddat$Voltage, type="l", ylab="Voltage", xlab="datetime",  xaxt = "n")
axis(1, xaxis, labels=xaxislabels)
#Third plot
...
#Fourth plot
plot(ddat$Global_reactive_power, type="l", ylab=names(dathead)[4], xlab="datetime", xaxt = "n")
axis(1, xaxis, labels=xaxislabels)
dev.off()
##################  End Plot 4 Charts ######################################
